...
public class NimUserInfoCache {
    final static String TAG = "NimUserInfoCache";
...
//                Log.i(TAG, "fetch userInfo completed, add users size =" + users.size());
    ...
    public List<NimUserInfo> getAllUsersOfMyFriend() {
        ...
        if (!unknownAccounts.isEmpty()) {
            DataCacheManager.Log(unknownAccounts, "lack friend userInfo", TAG);
...
//            LogUtil.e(TAG, "hasUser null, account=" + account + ", account2UserMap=" + account2UserMap);
    ...
    private void addOrUpdateUsers(final List<NimUserInfo> users, boolean notify) {
        ...
        DataCacheManager.Log(accounts, "on userInfo changed", TAG);
